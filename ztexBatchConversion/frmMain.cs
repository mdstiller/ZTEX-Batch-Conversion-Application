...
namespace ztexBatchConversion
{
    public partial class frmMain : Form
    {
        ...
        private uint uBitmapCode = 0;
        private string sFailedConversionPath = "";
        private bool bMoveFailedConversions = false;
        ...
        private void btnBrowse_Click(object sender, EventArgs e)
        {
            ...
            if (Properties.Settings.Default.SourceFolder.Equals(""))
                dlgFolderBrowser.SelectedPath = sBaseDirectory;
            else
                dlgFolderBrowser.SelectedPath = Properties.Settings.Default.SourceFolder.ToString();

            DialogResult dr = dlgFolderBrowser.ShowDialog();

            if (dr == DialogResult.Cancel)
                return;

            sOpenSelectedPath = dlgFolderBrowser.SelectedPath;
            Properties.Settings.Default.SourceFolder = sOpenSelectedPath;

            if (bMoveFailedConversions)
            {
                sFailedConversionPath = sOpenSelectedPath + "\\Failed Conversions";

                if (!System.IO.Directory.Exists(sFailedConversionPath))
                    System.IO.Directory.CreateDirectory(sFailedConversionPath);
            }
        ...
        private void convertZtexFiles(string sSelectedFile, int iFileNumber)
        {
            ...
            for (int index = 0; index < this.ztexFormat.Header.Count; ++index)
            {
                try
                {
                    ...
                    sExtractedFileName = (this.ztexFormat.Entry[index].Name).Replace(':', '_').Replace('/', '_') + ".png";
                ...
                catch
                {
                    ...
                    if (uBitmapCode == 0 || uBitmapCode == 3)
                        sFailedMessage = "failed - This program is unable to convert bitmap code " + uBitmapCode;
                    else
                        sFailedMessage = "failed - Bitmap code: " + uBitmapCode + ". Please report as bug";

                    bFailed = true;
                    sConversionStatusOutput = String.Format(sFormatString, DateTime.Now, sCurrentFile, sFailedMessage);
                    
                    if (bMoveFailedConversions)                    
                        System.IO.File.Move(sBaseFile, sFailedConversionPath + "\\" + sCurrentFile);
        ...
        {
            string sExtractionDetails = "";

            FolderBrowserDialog dlgFolderBrowser = new FolderBrowserDialog();
            dlgFolderBrowser.Description = "Select Destination Folder...";

            if (Properties.Settings.Default.DestinationFolder.Equals(""))
                dlgFolderBrowser.SelectedPath = sBaseDirectory;
            else
                dlgFolderBrowser.SelectedPath = Properties.Settings.Default.DestinationFolder.ToString();
            ...
            Properties.Settings.Default.DestinationFolder = sExtractSelectedPath;

            string[] oSelectedFiles =  Directory.GetFiles(sOpenSelectedPath);

            sExtractionDetails = "Converting files in folder " + getDirectory(oSelectedFiles[0]) + "\r\n"
                                 + "Storing converted files in folder " + sExtractSelectedPath + " \r\n";

            if (bMoveFailedConversions)
                sExtractionDetails += "Storing failed conversions in folder " + sFailedConversionPath + "\r\n";

            txtOutput.Text = sExtractionDetails;
        ...
        private Bitmap getTexBitmap(byte[] binary, int index, uint texFormat)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        case 0:
                            //tried A4, A8, ETC1, ETC1A4, HILO8, L4
                            return Textures.ToBitmap(binary, 0, (int)this.ztexFormat.Entry[index].W, (int)this.ztexFormat.Entry[index].H, Textures.ImageFormat.L4, false);
                        case 3:
                            //tried A4, A8, ETC1, ETC1A4, HILO8, L4, L8, LA4, LA8, RGB565, RGB8, RGB4, RGBA5551, RGBA8
                            //None work, format unknown
                            return (Bitmap)null;
                        case 7:
                            return Textures.ToBitmap(binary, 0, (int)this.ztexFormat.Entry[index].W, (int)this.ztexFormat.Entry[index].H, Textures.ImageFormat.A4, false);
                        case 9:
        ...
        private void moveFailedConversionsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ToolStripMenuItem oTemp = (ToolStripMenuItem)sender;

            if (oTemp.Checked)
            {
                oTemp.Checked = false;
                bMoveFailedConversions = false;
            }
            else
            {
                oTemp.Checked = true;
                bMoveFailedConversions = true;
            }
        }
        ...
        private void uncompressedBMPToolStripMenuItem_Click(object sender, EventArgs e)
        {
